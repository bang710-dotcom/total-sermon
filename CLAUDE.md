# CLAUDE.md — 토탈 설교 앱 (작업 메모리)

이 폴더(`total-sermon-web`)는 **정적 PWA "토탈 설교 앱"**의 배포 저장소다.
- 앱 본체: `index.html`(단일 파일, 메인 로직은 끝부분 큰 IIFE) · 서비스워커 `sw.js`.
- 백엔드: 구글시트 Apps Script 웹앱(`Code.gs`, 이 저장소가 아니라 `Dropbox/total-sermon/apps-script/Code.gs`에 보관). 델타 동기화(변경분만, `ID` 기준 멱등 upsert + 삭제로그 툼스톤).
- 로컬 저장: 설정·대기열·테마는 localStorage, 대용량 데이터(`data`)·`syncAt`·`seriesList`·`composeDraft`는 IndexedDB(`tsa`/`kv`).
- 자동 커밋·푸시 워처가 이 폴더에서 돌며 origin/main에 푸시 → 배포된다.
- 예화 데이터: `illustrations.json`(마스터색인 변환본, `total-sermon/tools/build_illustrations_json.py`로 재생성) + `illustrations-archive.json`(과거 원고 ex) 블록 스냅샷 — 예화 도서관 「내가 쓴 예화」 소스, 기기 보유 원고는 앱이 실시간 추출·대체). 상세: `Dropbox/total-sermon/docs/예화아카이브_통합_20260712.md`.
- **예화 샤드(v444+)**: 앱은 부팅 때 `ill-index.json`(story·sourceNote·source 제외 색인)만 파싱하고, 전문은 `ill-story-NN.<해시8>.json` 샤드로 백그라운드 병합(`illustLoad`/`illEnsureStories`/`illustLoadFull`). **illustrations.json 을 갱신하면 반드시 샤드도 재생성** — `build_illustrations_json.py`가 끝에서 자동 호출하며, 수동으로는 `python3 total-sermon/tools/build_ill_shards.py <illustrations.json 경로>`. illustrations.json 자체는 성경앱(CORS)·폴백용으로 계속 배포한다. 옛 해시 샤드는 생성기가 지우고, 기기 캐시는 앱이 청소.

## ⚠ 버전 규칙 (앱 코드 수정 시 항상 지킬 것)

앱 코드(`index.html`/`sw.js` 등)를 수정하면 **두 버전 번호를 항상 같은 값으로 함께 +1** 한다.

1. `sw.js` 의 `var CACHE = 'tsa-vNNN';`  — 서비스워커 캐시 무효화(새 코드 반영).
2. `index.html` 의 `var APP_VER = 'vNNN';` — 메뉴 하단 "앱 버전" 표시값.

→ 둘이 어긋나면(예: 캐시 v235 / 표시 v234) 실제 배포 버전을 화면으로 확인할 수 없다. **둘은 항상 동일 번호**여야 한다. 한쪽만 올리지 말 것.

## 요약본 두 종류 — `요약본` / `실제요약본` (v519+)

- **`요약본`** = 준비한 원고를 요약한 것(설교 상세 → 요약본·주제어).
- **`실제요약본`** = 유튜브 영상(실제로 선포한 설교)을 전사해 요약한 것. 서로 덮어쓰지 않는다.
- 밖으로 내보낼 때(성경앱 [설교] 노트·옵시디언·카드뉴스·공유·상태 배지)는 헬퍼 **`sumText(s)`**(=`실제요약본 || 요약본`)를 쓴다. 새 코드에서 `s['요약본']`을 직접 읽지 말 것.
- 시트 컬럼 `실제요약본`은 `SERMON_HEADERS` 맨 끝에 추가돼 있다 → **Apps Script `setup` 재실행 필요**.
- 유튜브 전사는 **앱에서 Gemini를 직접 호출**한다(설정 → Gemini API 키). 워커에서 부르면 `User location is not supported` 지역 차단이 나기 때문. 링크는 표준 `watch?v=` 형태로 정규화하고, 요청은 항상 10분 창으로 잘라 병렬 호출한다(통짜 요청은 500).

## 작성초안(`__작성초안__`) 우선순위 — v539+

미저장 작성 버퍼는 `시리즈초안` 시트의 예약행 1개(`__작성초안__`)로 기기 간에 오간다. **같은 설교 ID면 그 ID의 최종 저장본(설교 행 `수정일`)이 초안보다 우선**이다.
- `_cdraftAbsorbed(snap)` — 초안이 기반한 설교 행의 `수정일 ≥ snap.at + 2분`이면 "이미 저장에 흡수된 옛 초안"으로 보고 제안하지 않고 그 행을 지운다. 2분 여유는 기기 시계와 시트 서버 시각의 오차 때문(오판으로 흡수 처리하면 다른 기기의 미저장 원고가 사라지므로 보수적으로).
- `maybeOfferRemoteComposeDraft` — 예전엔 초안끼리만(`local.at` vs `snap.at`) 비교해서, 저장을 마쳐 로컬 초안이 빈 기기에서 옛 원격 초안이 항상 이기고 **최종본 이전 내용이 작성창을 덮어썼다**(2026-08-26 사례). 지금은 위 게이트를 통과한 초안만 배너로 제안하고, 배너에 초안 시각·시트 최종 저장 시각을 함께 보여 주며 **[최종 저장본 열기] / [초안 불러오기] / [무시]** 3지선다를 준다.
- `clearComposeDraft(true)`(저장 완료 시) → `dropComposeDraftRemote(true)` → `_sweepComposeDraftRemote()` — 로컬 캐시에 없는(아직 델타로 못 받은) 다른 기기 초안 행까지 시트에서 직접 받아 정리한다. 이 정리를 안 하면 그 행이 살아남아 나중에 되살아난다. 흡수되지 않은 진짜 미저장 초안은 건드리지 않는다.

## 산출물 파일명 — 시리즈 회차 번호 (v540)

파일명은 `sermonFileBase(meta, type)` 하나가 만든다: `YYYYMMDD 원고형태_예배구분_시리즈명+회차_제목_본문`.
- 회차 번호의 **정본은 별도 필드 `시리즈순번`**(입력칸 `#c_seriesNo`)이다. v539까지는 이 함수가 `시리즈` 문자열 끝의 숫자만 찾아서, 정상 입력(시리즈="요한복음" / 시리즈순번="3")이면 **번호가 통째로 빠진 파일명**이 나왔다. 지금은 `seriesTag(meta)`가 ①시리즈명 끝의 숫자(옛 입력 방식) ②`시리즈순번` 순으로 쓴다.
- **저장은 새 이름 하나로만**, **되읽기는 새·옛 이름 둘 다** 본다(`sermonFileBases`/`sermonFileNames`/`_nfcEqAny`/`_nfcInclAny`, `imgDataNames().jsonAlts·transAlts·backstagePrefixes`). 이 폴백을 지우면 v540 이전에 만든 **전환표·컷 사이드카·백스테이지 zip을 못 찾아 전환표가 새로 생성**되고 이미지 번호가 어긋난다.
- `docxFileName`(리더스앱 txt·폴더스캔 base)과 `ltcFileName`(LTC 교재)은 원래 시리즈를 넣지 않는 별도 규칙 — 건드리지 말 것.

## 검증 게이트 · 회중 반응 — v548

외부 설교 스킬 묶음(`cys-claude-sermon-skills`) 검토(`Dropbox/total-sermon/docs/외부설교스킬_검토_20260902.html`)에서 가져온 것들. 핵심 발상은 **AI가 쓴 답을 다시 AI에게 검사시키지 않고, 앱이 이미 가진 확정 데이터와 기계적으로 대조**하는 것이다.

- **성경 참조 실재성** — `VERSE_COUNTS_RAW`(66권 장별 절 수, 16진·약 3.3KB 상수) + `refAudit(text)` / `refAuditBare(list, bookIdx)` / `refAuditHtml()`. fetch 없이 즉시 판정한다. 자동 검사 지점: 본문연구 「AI 정리」(상호본문·클라이맥스·단락 범위), 「강해 검증」의 근거절, **완성 원고를 받은 직후**(`aiManuscript`). 수동은 작성 화면 [📕 성경 참조 전체 검증](`auditComposeRefs`).
  → ⚠ **`bible/bNN.json` 을 교체하면 이 표도 다시 뽑을 것.** 생성 명령은 `VERSE_COUNTS_RAW` 위 주석에 있다.
  → 오탐 방지: 절(`장:절`)이 있는 표기만 검사하고, 앞 글자가 한글·영문·숫자면(“그렇지요 3:16”) 건너뛴다.
- **사본 경계 본문 경고** — `MSS_DISPUTED`(막 16:9-20, 요 7:53-8:11, 요일 5:7-8 등 15곳) + `mssWarnFor()`. 작성 화면 본문칸 아래(`#c_mssWarn`, `renderMssWarn`)와 본문연구 창 머리(`#stMss`, `openStudy`)에 뜬다.
- **선택지 5결 매트릭스** — `MSG_DIVERSITY_RULE` 을 `P_SERMON`·`P_OCC`·`P_TOPIC`·`P_DAWN` 끝에 부착(`METAPHOR_AXIS_RULE` 뒤). 선택지가 서로 다른 결(신학명제/실천권면/내러티브/실존진단/종말소망)에서 나오게 하고, 핵심메시지 25~45자·추상명사 나열 금지를 함께 건다.
- **찬송 번호 대조** — `hymnFixSync()` 가 `sanitizeHymns()` 안에서 성경앱 645장 목록과 맞춘다. **사용자 입력용 `hymnResolve` 와 우선순위가 반대다** — 모델은 곡명이 정확하고 번호를 자주 틀리므로 여기서는 곡명 우선, 목록에 없는 번호는 떼어 낸다. 호출 전에 `await studyLoadHymns()` 로 목록을 확보해 둔다(목록이 없으면 손대지 않는다).
- **흔한 오용** — `MISUSE_RULE` 을 `P_STUDYSYN` 에 부착하고 출력 JSON에 `흔한오용[]`(주장·유형·바로잡기) 추가 → `renderStudyAi` 의 「⚠ 이 본문의 흔한 오용」.
- **회중 반응 시뮬레이션** — `PERSONAS_ADULT`(전체 회중 8인)·`PERSONAS_YOUTH`(대학부 8인) 두 벌 + `P_PERSONA` + `aiPersona(kind)` / `renderPersona`. 「강해 검증」이 *원고가 본문에 맞는가*를 보는 데 비해 이건 *누가 어떻게 듣는가*를 본다. 페르소나 목록이 **유일한 발화 근거**이고, 페르소나끼리 대화 금지·신학 수준 초과 발화 금지·없는 일화 창작 금지를 프롬프트에서 못 박는다. 페르소나를 고치려면 두 배열만 손대면 된다.

## 집중모드 본문 열 좌우 이동 — v549

모니터 두 대를 쓰면 브라우저가 놓인 화면이 시선에서 비껴 있어, 그 화면 "가운데"의 본문을 보려고 고개를 계속 돌려야 한다. 그래서 집중모드 본문 열을 좌우로 옮길 수 있게 했다.

- **`--fmShiftX`(CSS 변수) + `body.focusMode #editorPane{position:relative;left:var(--fmShiftX,0px)}`.
  ⚠ `transform` 은 쓰지 말 것** — 진입·종료 FLIP 애니메이션(`fmFlip`)이 `#editorPane` 의 `transform` 을 인라인으로 쓴다. 둘이 겹치면 전환이 깨진다.
- 조작 셋: 툴바 `◀ ⊙ ▶`(`data-act="shiftL/shiftMid/shiftR"`, `.fbPos`) · 빈 여백 마우스 드래그(더블클릭=가운데) · `⌃⌥←`/`⌃⌥→`/`⌃⌥0`.
  → 단축키에 `⌥←/⌥→` 단독을 쓰지 않은 이유: macOS에서 **단어 단위 커서 이동**이라 타이핑을 방해한다.
  → 드래그는 **pointer 가 아니라 mouse 이벤트**다. `mousedown` 의 `preventDefault` 는 선택·포커스 이동만 막고 `click`·`dblclick` 은 그대로 흘려보내, "끌어서 옮기기"와 "더블클릭해서 가운데로"가 함께 동작한다(pointerdown 에서 막으면 브라우저마다 뒤따르는 마우스 이벤트가 갈린다). 마우스 전용이라 아이패드 스크롤 제스처는 건드리지 않는다.
- 함수: `fmShiftMax()`(열이 화면 밖으로 안 나가는 한계 = 좌우 여백) · `fmApplyShift(px, save)` · `fmCurShift()` · `fmNudgeShift(d)`. 값은 `localStorage['tsa.fmShiftX']` 에 **기기별로** 둔다(사무실·집의 화면 배치가 다르므로).
- 창을 좁히면 `resize` 에서 다시 가두되 **저장값은 건드리지 않는다** — 넓은 화면으로 돌아오면 원래 위치가 복원된다. 복원은 `fmRestorePrefs()` 안에서.
- 툴바의 위치 버튼은 `@media (max-width:900px)` 에서 숨긴다(옮길 여백이 없는 화면에선 의미가 없다).

## 삽입 예화 원본 보기 — v550 (v551에서 버튼 위치 이동)

오른쪽 작성창 예화 칸의 「예화 [AI 생성]」 라벨 줄, **[AI 생성] 바로 오른쪽**에 **[📖 도서관]** 버튼이 붙고, 누르면 도서관 원본(전문·강조메시지·주제/층위 배지·연결 본문·출처·사용된 설교)이 모달로 뜬다.
- v550 에서는 왼쪽 구조화 창 예화 블록의 제목 칸 옆에 뒀으나, 그 열은 폭이 좁아 예화 제목이 가려져 v551 에서 오른쪽 작성창(`renderEditor`)으로 옮겼다(`.edIllLib`, 집중모드에서는 `.edBlkAi` 와 함께 숨김).

- 모달: `#illViewOv`(마크업) + `illViewOpen/illViewFind/illViewHtml/illViewClose`. `#illOv` 와 같은 CSS·z-index(340)를 쓰므로 모달 안의 성구 링크(`#brefOv`, 350)가 그 위에 정상적으로 뜬다. `anyOverlayOpen()` 목록에도 등록.
- 여는 키는 `illBlockLibKey(b)` — ①`srcKey`(도서관에서 삽입한 블록의 정본) ②없으면 **제목 완전일치 폴백**. 저장했다 다시 연 원고는 `editSermonFill`→`parseFreeToStructure` 가 원고 텍스트에서 블록을 다시 만들어 `srcKey` 가 없기 때문에 폴백이 없으면 버튼이 사라진다. 우연 일치("살아계신 하나님" 류)는 `illStoryOverlap` 으로 걸러낸다 — `illComposeUsedLibIds` 와 같은 규칙이며, 제목 색인은 두 함수가 `illNormIdx()` 로 공유한다.
- 전문(story)은 샤드에 있으므로 `illustLoadFull()` 을 기다린 뒤 `illUnified()` 로 다시 찾는다(부팅 직후 색인만 있는 상태에서 빈 본문이 뜨지 않게). `arc:` 키는 `illArcLoad()` 에서 찾는다.

## 미리보기 쪽번호 — v552

미리보기 종이 페이지마다 **아래 여백 한가운데에 `현재 / 전체` 쪽번호**를 찍는다(창보기·전체화면 공통, 인쇄에도 그대로 나감).
- `.pv-pgno`(CSS) + `openPreview()` 페이지 조립 루프에서 `.pv-page` 안 마지막에 삽입. **본문 흐름 밖 절대배치**라 다단(column) 폭 계산·줄바꿈·페이지 수에 영향을 주지 않는다 — 이 원칙을 깨고 흐름 안에 넣으면 미리보기와 내보내기 결과가 어긋난다.
- 위치·크기는 페이지 규격에서 계산한다: `noFs=11px`, `bottom = max(4, pb/2 - noFs/2)` → 아래 여백(10mm·12.5mm 둘 다) 한가운데. `.pv-page` 가 이미 `position:relative` 라 별도 처리 불필요하고, `transform:scale(k)` 를 같이 받아 확대·축소해도 비율이 유지된다.
- 전체화면에서 잠깐 뜨는 floating 배지 `#pvPageNo` 는 **그대로 둔다** — 페이지가 화면보다 길면 종이 아래쪽 쪽번호가 시야 밖이라 빠르게 넘길 때는 배지가 여전히 필요하다. 인쇄 CSS의 `#pvPageNo{display:none}` 은 배지(id)만 가리며 `.pv-pgno`(class)와 무관하다.

## 이미지 전환표 = 독립 단계 — v553 (v554에서 즉시 열기)

전환표는 **원고 내보내기의 부산물이 아니라 스스로 하나의 단계**다. 중간 단계 맨 위(HWPX 원고 버튼 아래)의 **[🔀 이미지 전환표 생성/수정]**(`data-casc="transtable"` + 토글 `transtable`)에서 만든다.

- **왜 옮겼나**: v552까지는 `exportManuscriptDocx()` 끝에서 무조건 `ensureTransTable` 을 불렀다. 진행 표시는 「원고 내보내는 중」인데 뒤에서 원고 전문을 넣은 **Opus 호출**이 돌았고(`cascStepList` 에도 없는 숨은 단계였다), 이미지를 안 쓰는 원고에서는 그대로 낭비였다. 원고는 내보낸 뒤에도 계속 고쳐지므로 **"이미지를 쓰기로 확정한 시점"의 원고로 한 번만** 잡는 편이 맞다.
- **의존성(오류 방지)**: `CASC_DEPS` 로 이미지 가지 4종(`trans`·`imgdata`·`bgimg`·`transpdf`)을 켜면 `transtable` 이 **자동으로 켜지고**, `CASC_DEPENDENTS` 로 `transtable` 을 끄면 그 4종이 **함께 꺼진다**. `cascSel()`·저장값 로드에도 같은 보정이 있어 어느 경로로도 "전환표 없이 이미지 산출물" 조합은 나오지 않는다.
- **카드 가지는 전환표를 강제하지 않는다** — 이미지 없이 카드원고만 뽑는 경우를 위해서다. 켜고 끄는 판정은 `cascWantTrans(target, sel, inA, inB)` 하나가 하고(`cascStepList`·`runCascade` 공용), 꺼진 채 카드를 뽑으면 `window._cascNoTrans` 가 서서 **`ensureTransTable` 이 있는 표는 쓰되 새 표는 만들지 않는다**(AI 호출 없음).
  → ⚠ 이때 카드원고의 빨간 전환 바는 모델이 자체적으로 넣고 `renumberCardTransBars(arr, null)` 가 #2부터 순번만 매긴다. **바를 아예 빼려면 별도 처리가 필요하다**(현재는 넣는 쪽이 기본).
- 완료 점(`cascDone.transtable`)은 시트 산출물이 아니라 **파일맵의 표 보유 여부**(`sermonTrans(id)`)로 판정한다.
- **Total 은 전환표만 한 번 묻는다 (v555)** — Total 은 원래 켜둔 산출물을 전부 새로 만들지만(`_cascForce`), 전환표는 따로 잡아 둔 표에 맞춰 이미지를 이미 만들어 둔 경우가 많다. 그래서 전환표 단계가 포함되고(`wantTrans`) 저장된 표가 있을 때만 「생성된 전환표가 있는데, 전환표부터 다시 생성할까요?」를 묻고, **[기존 전환표 유지]** 를 고르면 `window._cascKeepTrans` 가 서서 `ensureTransTable` 의 `force` 만 풀린다(`force = _cascForce && !_cascTableFresh && !_cascKeepTrans`). 카드·컷 등 나머지는 `_cascForce` 그대로라 새로 만들어지고, 새 컷도 유지된 표의 번호를 따르므로 **이미 만든 배경 이미지와 번호가 계속 맞는다**. 취소(바깥 클릭·ESC)는 "유지" 쪽 — 잃는 것이 없는 선택이 기본값이다.
  → 단일 버튼([배경 이미지 생성] 등)은 애초에 `_cascForce` 를 켜지 않으므로 묻지 않고 그냥 기존 표를 쓴다.
- **옛 [🔎 이미지 전환표 보기 · 다시 잡기] 버튼은 없앴다** — 이 버튼이 흡수했다. 한 번 누르면 항상 표 앞에 선다.
  - **표가 이미 있으면 캐스케이드를 아예 타지 않고 `transTableOpen()` 만 즉시 호출한다(v554, 클릭 핸들러에서 분기).** 캐스케이드를 태우면 오버레이·폴더 확인에 더해 끝에서 **`pullData()`(시트 델타 동기화)** 까지 기다리느라 창이 몇 초 뒤에 떴다. 수정은 창 안의 [🔄 다시 잡기]·[📂 되읽기]로 한다.
  - 표가 아직 없을 때만 캐스케이드로 생성하고(AI 호출), 끝나면 창을 연다. 이때는 "이미 만들었어요?" 3지선다를 건너뛴다.
  - 즉시 열기는 `ensureTransTable` 의 재정렬을 건너뛰므로, 표를 잡은 뒤 원고가 바뀌었으면 창 머리에 경고 배너를 띄운다(`transStale()`/`transStaleHtml()` — 기록이 없으면 조용히 넘어간다). 배너는 [♻ 원고 수정 반영](위치만 재정렬)과 [🔄 다시 잡기](새 표)를 안내한다.
- **전환표를 끈 채 만든 카드류는 전환 바가 빠진다**(`cardNoTrans()`/`stripCardTrans()`): 카드 원고에서 `t==='trans'` 블록 제외, 디자인 PDF·PPTX 는 카드 모서리 이미지 번호 배지(`deck.noImgBadge`) 숨김. **저장본(파일맵 `cardArr`)에는 바가 그대로 남고 "내보낼 때만" 걸러낸다** — 나중에 이미지를 쓰기로 하고 다시 뽑으면 같은 카드에 바가 되살아난다(캐시 오염 없음).
  → ⚠ `buildCardDocx` 의 쪽나눔은 *일반 전환 바*를 덩어리 경계로 쓰는 `keepNext` 연쇄다. 바가 하나도 없으면 문서 전체가 한 덩어리로 묶여 쪽나눔이 망가지므로 **`hasTrans` 가 false 면 keepNext 를 걸지 않는다**(자연 흐름). 이 가드를 지우지 말 것.

## 강단 스크린 두 벌 출력 — v556

강단 스크린은 셋이다 — **가운데 초광각 4160×1620 한 대 + 양옆 16:9 두 대**. 예전에는 같은 16:9 이미지를 가운데 화면에 늘여 써서 좌우로 왜곡된 그림이 나왔다. 이제 한 번 생성한 배경에서 **가운데용 초광각 원본**과 **사이드용 좌우 크롭본**을 함께 만든다.

- 설정 → 🖼️ 설교 이미지 AI 선택 → **🖥 강단 스크린 출력**(`imgScreens`): `both`(기본) / `wide` / `side`. `side` 는 v555까지의 동작과 같다.
- 생성 요청 비율 자체가 초광각(`imgGenAR()` → `"4160:1620"`)이라 **AI가 처음부터 넓은 화면을 구상**한다. 프록시 워커가 임의 `W:H` 를 받도록 함께 고쳤다(아래).
- 컷마다 「텍스트 없는 배경」을 먼저 정하고 **화면 규격마다 `fitCanvasCover()` 로 잘라 맞춘 뒤 각각 합성**한다(`generateImageSetFromCuts`). ★초광각에 맞춘 글자를 나중에 자르면 좌우가 잘려 나가므로 **텍스트 합성은 반드시 규격별로** 해야 한다. `fitCanvasCover` 는 늘이지 않고 가운데 기준 crop(cover) — 실측 검증: 원본의 정사각형이 두 판 모두 정사각형(비율 1.000)으로 남고, 사이드에서만 좌우 표식이 잘린다.
- **파일 배치** — zip 루트 = 기본 세트, 하위 폴더 = 보조 세트.
  - `both`: 루트 `NN.jpg`(초광각) + `side-1920x1080/NN.png`(사이드). 폴더명은 맥 NFD 문제를 피해 ASCII.
  - `wide`: 루트 `NN.jpg` / `side`: 루트 `NN.png`(예전 그대로)
- **초광각이 JPEG(q0.94)인 이유**: 4160×1620 PNG 는 실측 13.2MB/장 → 20장에 264MB 로 zip 이 감당 못 한다. JPEG 는 1.3MB/장(성구 컷 0.26MB). 사이드는 PNG 유지 — zip 루트를 읽던 기존 코드와 호환.
- 읽는 쪽 세 곳을 함께 고쳤다: `tpdfImgMap`(png·jpg 모두 인식 + **루트만** 써서 중복 방지), `zipCutFile()`/`zipDropCut()`(낱장 재생성이 확장자·폴더 무관하게 찾고 지움), 낱장 재생성도 **모든 규격을 함께 갱신**한다.
- **표지도 일반 컷과 같은 모델** (v557) — 예전엔 표지만 `background-hero`(Flux=`max`)로 뽑았다. 표지는 설교 시작에 잠깐 띄우는 화면이라 그만한 품질·비용·시간이 필요 없고, 초광각으로 화소가 커지자 **그 느린 모델이 워커 폴링 예산을 넘겨 #01에서 생성이 멈췄다**. 지금은 `engineRoute('background')` 로 통일(Flux=`klein`). `background-hero` 라우팅 자체는 프록시에 남아 있다(설정의 연결 테스트에서 선택 가능).
- **멈춤 방지 3종** (v557) — ① `genSermonImage` 에 210초 AbortController 타임아웃(예전엔 무제한이라 프록시가 응답을 안 주면 화면이 영원히 멈췄다) ② `genBgWithRetry`/`genCoverWithRetry` 가 오류를 삼키지 않고 `imgNoteErr()` 로 콘솔·진행 줄·`_imgLastErr` 에 남긴다(폴백 프롬프트로 조용히 재시도하느라 멈춘 것처럼 보이던 문제) ③ 워커 Flux 폴링 60초→120초, 실패 메시지에 치수·마지막 상태 포함.
- **초광각 화소 예산** — 워커 `arToWH(ar, 1.6)`. 처음에 2.0MP 로 잡았더니 기존 16:9(1.33MP) 대비 +55% 라 생성이 느려져 타임아웃을 유발했다. 지금은 Flux **2048×800**(1.64MP, +23%). Krea 는 2.3MP 유지(2464×960).
- ⚠ 프록시 워커(`Dropbox/total-sermon/image-proxy/worker.js`)를 **수동 배포해야** 초광각이 실제로 나온다(`npx wrangler deploy` — 이 맥에는 Node 가 없어 npx 가 자동 설치한다). 배포 전에는 워커가 초광각 요청을 16:9 로 떨어뜨리므로, 앱이 그 16:9 를 위아래로 잘라 초광각을 만든다(왜곡은 없지만 화각이 좁다).
- **가장자리 비네팅은 기본 끔** (v559, `imgVignette` · 설정 → 🌑 가장자리 비네팅). 원래는 16:9 한 장을 전면 와이드에 띄울 때 **좌우 검은 여백과의 경계를 감추려던** 처리였는데, v556부터 전면·사이드를 각각 제 비율로 뽑아 여백 자체가 없어져 필요가 없다. `processGeneral` 이 켜졌을 때만 `applyVignette` 를 태우고, 꺼져 있으면 `imgToCanvas` 로 원본 그대로 넘긴다(밝기 감마 보정은 켜짐 여부와 무관하게 유지).
  → **`LED_TAIL` → `screenTail()` (v560)**: 이게 진짜 원인이었다. 모든 생성 프롬프트 꼬리에 «Strong dark vignette on all edges, left and right sides fading completely into pure black … Widescreen 16:9» 가 붙어 **AI가 그림 자체에 비네트를 그리고 16:9로 구도를 잡고 있었다** — 캔버스 비네팅만 꺼서는 소용이 없다. 지금은 설정을 따라 «프레임을 가득 채우고 검은 띠·레터박스·비네트 없음» + 비율(초광각 2.57:1 / 16:9)을 지시한다. **'두 벌 모두'일 때만** «주제와 가장 밝은 빛을 가운데 16:9 영역 안에» 를 덧붙인다 — 사이드가 가운데를 잘라 쓰므로 주제가 좌우 끝에 몰리면 잘려 나가기 때문.
  → **비네팅을 켜면 프롬프트가 「가운데로 모으라」로 바뀐다 (v561)**. 앱이 생성 *뒤에* 가장자리를 죽이므로, 그 사실을 프롬프트에 알리지 않으면 주제가 비네트에 묻힌다. 안전 구역 숫자는 **실측값**이다 — 마스크(타원 x13~87%·y10~95% + 폭 7.5% 블러)를 흰 판에 씌워 재면 **원본 밝기 80% 이상을 유지하는 구역이 가로 52%·세로 50%** 뿐이고, 가로 70% 지점은 이미 43%까지 떨어진다. 그래서 «중앙 절반 이내»로 지시한다. **이 숫자를 키우면 주제가 묻히므로 `applyVignette` 의 타원·블러를 고칠 때 함께 다시 재야 한다.**
  → 이미지자료 문서(`hwpx`/`docx`)의 한국어 안내도 `screenNoteKo()` 로 같은 기준을 따라간다(모드별 문구 + 비네팅을 켜 뒀으면 그 사실도 표기). 두 문서 빌더(`buildImgDataParas`·docx 판)가 이 헬퍼 하나를 공유하므로 한쪽만 낡는 일이 없다.
- ⚠ 성구 글자 크기는 `h*0.052` 기준이라 초광각 판에서는 폭 대비 작게 보인다. 필요하면 `drawScriptureText` 에 폭 기준 하한을 넣을 것(현재는 손대지 않음 — SCRIPTURE_TEMPLATE 실측 규격 유지).

## 지난 설교 예화 회고 — 수정 전 예화가 뜨던 문제 (v563)

원고에서 예화를 바꿔 저장해도 **수정 전 예화**가 홈의 [🎤 지난 설교 예화 회고]에 떴다. 원인은 사용 기록이 **누적 전용**이었던 것 — `예화` 요약 필드는 저장할 때마다 현재 대지에서 새로 만들어지는데(`collectCompose`), 기록(`사용설교ID`·`illUse`)은 추가만 하고 빠진 예화에서 설교 ID를 빼지 않았다.

- **원천 수정** — `illRecordUsage(sermonId)` 가 이제 그 설교에 대해 **지금 원고의 예화 집합과 일치**시킨다(빠진 예화에서는 설교 ID 제거). 인박스는 `사용설교ID`, 라이브러리는 새 `illUseUnrecord()`.
- ⚠ **`composeIllTrustworthy()` 게이트** — 구조에 예화 블록이 0개인데 원고 본문에 `ex)` 가 있으면 자유원고 파싱이 실패한 상태다. 그때 지우면 멀쩡한 이력이 통째로 날아가므로 **추가만 하고 삭제는 건너뛴다.** 이 게이트를 지우지 말 것.
- **표시 단계 안전망** — `renderIllRecap` 의 `stillInSermon(it, s)` 가 후보를 그 설교의 **현재 `예화`·`원고텍스트`** 와 대조해 없으면 뺀다. 이미 쌓인 옛 기록과 **다른 기기에서 동기화돼 들어온 기록**까지 걸러 준다(대조할 내용이 아예 없으면 예전처럼 통과).
- ⚠ 남은 한계: `illUsePull` 이 기기 간 기록을 **합집합**으로 병합하므로, 다른 기기가 옛 기록을 갖고 있으면 로컬에서 지운 것이 다시 살아날 수 있다. 표시 단계 안전망이 그 경우를 가려 주지만, 근본 해결은 툼스톤이 필요하다.

## 버튼 작동 중 표시 — `withBusy` (v565 · v566)

「본문연구 → AI 정리 → 📖 성경앱 노트로」가 성경앱 시트 입력이 끝날 때까지 **아무 반응이 없었다**. 두 가지가 겹쳤다 — ① `studyToBible()` 이 프라미스를 **돌려주지 않아** 버튼이 완료를 기다릴 수 없었고 ② 느린 이유는 기존 노트의 `createdAt` 을 보존하려고 **성경앱 메모 전체를 먼저 받아 온 뒤**(`tsaMemoLoad`) upsert 하기 때문(Apps Script 왕복 2번).

- 공용 헬퍼 **`withBusy(btn, label, fn)`** — 스피너 + 라벨 교체 + `.busy` + **`disabled`**(v565에서 추가, 키보드 Enter 재실행까지 차단) + `dataset.busy` 연타 방지, 끝나면 원래대로 복원. **fn 은 반드시 프라미스를 돌려줘야** 한다(안 돌려주면 즉시 풀린다). 위임 클릭에서는 `withBusy(clickedEl, '…중', fn)()` 처럼 즉시 호출.
- 이미 쓰는 곳: `#checkInbox`·`#outRefresh`(이번에 disabled 도 함께 받음)·`.stPipeBible`.
- **이미 자체 피드백이 있는 경로**(여기엔 `withBusy` 불필요): `submit()`→`showWaitOverlay`(시트 저장 전반) · `callClaude()`→`showGenOverlay`(AI 전반) · `runCascade`→캐스케이드 오버레이 · `loadSermonThen`→`showLoad` · `devoMigrateToBible`/`bmPushAllSermons`→`showWaitOverlay`.
- **전수 조사(2026-09-11) 결과 — 아직 피드백 없이 기다리는 버튼**(정적 분석 270개 트리거 → 후보 29 → 코드 확인):
  1. ~~Holman 별항 링크(`.hbFeat`→`openFeat`)~~ → **v566 처리**: 아직 안 받았으면 창부터 띄우고 「별항을 불러오는 중…」, 받는 사이 창을 닫으면 다시 열지 않음, 실패·없음은 창 안에 표시.
  2. ~~동기화 대기열 「지금 전송 시도」(`#qFlushNow`)~~ → **v566 처리**: `withBusy(f, '전송 중…', …)` — 끝날 때까지 비활성 + 스피너.
  3. 이달의 묵상 ◀ ▶ 오늘 — 화면은 즉시 바뀌지만 성경앱 노트 칸이 나중에 조용히 채워짐(경미).
  4. 시리즈 「요청 결과」 — 로컬 폴더 스캔만 기다림(대개 즉시, 경미).
  → 나머지 25건은 오탐(fire-and-forget 푸시 · 모달이 먼저 뜨고 「불러오는 중…」 표시 · OS 폴더 선택창 자체가 피드백).
- **「성경앱 노트로」 가속 (v566)** — 느린 원인은 메모 전체 다운로드(`tsaMemoLoad`)였다. 이걸 **없앨 수는 없다**: 성경앱 메모 시트 서버(`Dropbox/bible/sermon-sync/memo_sheet.gs` 의 `_rowFor`)가 `m.createdAt || now` 로 **행 전체를 덮어쓰므로**, 목록 없이 보내면 기존 노트의 작성일이 지금으로 바뀐다. 그래서 **AI 정리가 화면에 뜨는 순간 목록을 미리 받아 둔다**(`studyPrefetchMemos()`, AI 창 렌더 끝에서 호출). 사용자가 결과를 읽는 동안 받아지고, 누를 땐 upsert 1회만 기다린다. `tsaMemoLoad` 가 프라미스를 캐시하므로 받는 중에 눌러도 같은 요청을 이어받는다. 실측(가짜 지연 GET 1.5s·POST 0.3s): **미리 받은 뒤 322ms vs 예전 1825ms**, 누를 때 추가 목록 요청 0건.
  → 완전히 없애려면 서버 `_rowFor` 가 기존 행이 있을 때 비어 있는 `createdAt` 을 **기존 값으로 보존**하도록 고쳐 재배포해야 한다(그러면 앱은 목록 없이 바로 upsert 가능). 같은 패턴인 묵상·설교 노트 전송도 함께 빨라진다.
  → `bmUpsert` 는 성공 후 `STUDY._memoP = null` 로 캐시를 비운다(다른 기기 변경을 놓치지 않으려는 의도) — 같은 구간을 연달아 두 번 보내면 두 번째는 다시 목록을 받는다.

## 주보내용 공유하기 — v567

작성 화면 **[📮 주보내용 공유하기]**(`#c_kakao`, 옛 이름 「주보내용 카톡 초안」)는 결단찬양만 묻고 **곧장 기기 공유 시트**를 띄운다(`bulletinKakao` → `bulletinShare`).
- 앱 안 초안 모달(`bulletinDraftOpen`)은 없앴다 — 카카오톡을 고르면 카카오톡 공유 창이 다시 내용 수정 칸을 보여 줘서 **수정 과정이 두 번**이었다. 수정은 공유 대상 앱 창에서 한 번만 한다. 고친 초안 보관(`파일맵.bulletin`)도 함께 없앴다.
- 찬송가 목록·성경 색인을 받는 사이 Safari 가 사용자 동작 만료로 `share()` 를 거부하면(`NotAllowedError`) 그때만 작은 [↗ 공유하기] 창을 띄워 한 번 더 탭을 받는다(카드뉴스 `cnPresentShare` 와 같은 이유).

## 성경 인용 빨간 볼드 — 판별 규칙 확장 (v570 · v571)

미리보기·원고 내보내기(hwpx·docx)는 성경 인용 줄을 빨간 볼드(`REF_RED`)로 찍는다. 판별은 `refQuoteRe()` 하나였는데 **「약어 장:절, "」 처럼 쉼표가 필수**라, 실제 원고에 더 흔한 「수 1:5-9 “…」(쉼표 없음)가 검정으로 나왔다(2026-09-20 원고는 빨간 줄이 0개였다). 여러 줄 인용(「6 강하고…」「7 오직…」)은 성구 표기가 없어 처음부터 잡을 수 없었다.
- `refQuoteRe()` — 쉼표 선택, 여는 자리의 `”` 오타 허용, 장을 넘는 범위(`5:13-6:7`), 책 이름 없는 `7:11 “`, 절만 쓴 `1절, “`·`11-12절, “`.
- **`refQuoteTracker()`** — 줄을 차례로 넣는 상태 판별기. ①위 형식으로 시작하는 줄 ②성구만 있는 줄(`bareRefRe`, 「대상 12:32」) **바로 다음의** 따옴표로 시작하는 줄 ③닫히지 않은 인용의 이어지는 줄 — **절 번호로 시작하거나 닫는 따옴표가 있는 줄만** 잇는다(닫는 따옴표를 빠뜨려도 설교 전체가 빨개지지 않게). 빈 줄은 상태를 지운다. `parasFromText`·`parasFromCompose` 가 이걸 쓰며, **새 코드에서 `refQuoteRe().test()` 를 줄 단위로 직접 부르면 여러 줄 인용을 놓친다.**
- 검증(2026년 원고 15편, 목사님이 한글에서 직접 칠한 빨강과 대조): 예전 규칙이 놓친 인용 **50줄**을 새로 잡고, 검정이어야 할 줄을 빨갛게 만든 경우 **0건**(「손은 검정·새 규칙은 빨강」 7줄은 전부 실제 인용 — 5줄은 이 버그로 검정이 된 9/20 원고).
- ⚠ 남은 한계: 성구 표기가 **아예 없는** 절 번호 줄(「4 사랑은 오래 참고…」) — 대지 번호 등과 구별할 근거가 없다.

### v571 — 부분 색칠(인용 구간만 빨간 볼드)
v570은 문단 단위로만 칠해서 ①「수 6:18a "…". 어떤 것이든지…」처럼 **인용 뒤 설명까지 통째로 빨갛고** ②「…보십시오. 수 7:11a "…". 아간…」처럼 **문장 중간 인용은 못 칠했다**(2026-09-21 제보).
- `refQuoteTracker()` 가 이제 `null`(인용 없음) / `true`(줄 전체 — 남는 게 공백·문장부호·」』뿐) / `[[글, 인용?], …]`(부분) 을 돌려준다. 인용 구간 = **성구 표기부터 닫는 따옴표까지**(`refAnyRe()` 로 줄 어디서든, 한 줄에 여럿도). 한 성구에 인용이 `/`·쉼표로 이어지면(「행 1:6-7 "…?" / "…"」) 한 구간으로 묶는다.
- 문단 모델에 **`spans`** 필드가 생겼다(부분 인용 문단에만). 네 소비처가 모두 읽는다 — 미리보기 `paraPreviewHtml` · HWPX `hwpxParasToBlocks`(`runs` 로 `red`/`normal` 스타일) · DOCX `buildDocxParas`(TextRun 여러 개) · 전환 PDF `tpdfBlocks`. **새 소비처를 만들면 `spans` 를 처리해야 한다** — 무시하면 `text` 로 떨어져 검정 문단이 된다(안전하지만 색이 빠짐).
- 부분 인용 문단은 일반 문단 서식(첫 줄 들여쓰기)을 유지하고 인용 구간만 빨간 볼드. 줄 전체 인용은 예전처럼 들여쓰기 없는 빨간 볼드 문단.
- 칠하지 않는 것(의도): 성구와 따옴표 사이에 말이 끼면(「4절을 보십시오. “…”」) 인용으로 보지 않는다 — 「시 89:1에서 …」 같은 언급과 구별할 근거가 없어서.

## 작성창 유실 방지 — v572 (수정계획 1단계)

근거: `Dropbox/total-sermon/docs/수정계획_20260921.md` 1단계 · `앱_전반점검_20260921.md` 1-1·1-4·1-5. (계획서의 "v570"은 v570·v571이 빨간 볼드에 쓰여 **v572**가 됐다.)
- **왕복 가드(핵심)** — `editSermonFill` 은 `parseFreeToStructure()` 뒤 **`structRoundTripOk(free)`**(재조립 결과를 `rtNorm` 으로 원문과 비교: 줄 앞뒤 공백·빈 줄·`ex)`/`(예화)` 표기만 무시)가 참일 때만 기본원고로 연다. 아니면 자유원고 — 결론 머리가 없는 옛 원고는 구조로 열었다 저장하는 순간 「말씀을 정리합니다.」+대지 요약이 붙는 등 본문이 바뀌었다. **이 가드를 지우지 말 것.** 앱에서 작성·조립된 원고는 그대로 기본원고로 열린다(회귀 테스트 397/397).
- 파서: 결론 머리는 **줄머리** `(결론)`·`말씀을 정리…` 만 · 첫 대지 이후 번호 머리는 **첫 대지와 같은 구분자(`.`/`)`) + 다음 번호**일 때만 대지(소목록 승격 방지) · `【`·`▶` 줄은 본문에 보존 · `assembleText` 는 끝 줄이 이미 핵심메시지면 다시 붙이지 않는다.
- 불러온 설교도 `_freeSnap`/`_structSnapSig` 를 잡는다(칩 전환 포함, 서명은 **`renderEditor()` 뒤** — 결론 칸 시드가 서명을 바꾸므로).
- 자유원고 저장의 `대지`·`예화` 컬럼과 예화 사용 기록은 **`composeEffectivePoints()`**(자유원고면 작성 버퍼를 건드리지 않는 임시 파싱본 `composeFreePoints()`, 메모이즈)에서 뽑는다. 임시 파싱이 실패하면 `composeIllTrustworthy()` 가 false → 기록 삭제 건너뜀.
- 원고유형 칩: 전환 전 미저장(`wasDirty`)이면 `savedSig` 를 다시 잡지 않는다.
- 저장 서명은 **수집 직후**(`_sig0`)에 잡아 `finishSave`·`saveReconcileCopy(…, {sig, cut})` 로 넘긴다(저장 직전 동기화 사이 친 글자 보존). 45,000자 초과는 `confirmCutManuscript()` 로 먼저 묻고, 잘라 저장했으면 미저장·초안을 유지한다.
- 원격 초안 [초안 불러오기] 전 `confirmDiscardCompose`, `restoreComposeDraft` 시작 때 `remoteConflict=null`. ⌘S 는 `e.code==='KeyS'` 도 본다(한글 자판). 집중모드 Esc 는 IME 조합 중·확인창(`body.uiDlgOpen`)일 때 무시.
- **회귀 테스트**: `Dropbox/total-sermon/tools/roundtrip_test.py`(smoke.sh ④가 자동 실행) — `sermons_import.json` 1,121편을 파싱→재조립. 「구조로 열림 중 불일치」가 0이 아니면 실패. smoke.sh 는 node 가 없으면 jsc `checkSyntax` 로 폴백하고, 고아 예화 샤드를 나열한다.

## 원고 버전 이력 — v573 (수정계획 2단계)

작성 화면 [저장] 줄의 **[🕘 이전 버전]**(`#c_hist` → `#histOv`). 원고가 통째로 바뀌는 순간마다 작성 버퍼(`state.compose` 전체 + 메타 칸 + `assembleText()`)를 **이 기기 IndexedDB `tsa/kv` 의 `hist:<설교ID>`**(새 설교는 `hist:_new`)에 남긴다. 시트로는 보내지 않는다.
- 스냅샷 지점: 저장 성공 뒤(`histAfterSave` — 저장·새 설교로 저장·검토본 저장, 내용은 **저장 데이터를 모은 순간**의 `histCapture`) · `aiManuscript` 반영 직전 · 대화창 [작성칸에 반영] 직전 · 원격 초안 [초안 불러오기]·`loadRemoteConflict`·`reconcileCleanDraftWithSheet` 자동 교체 직전 · 되돌리기 직전.
- 규칙: 설교당 최근 10개(`HIST_MAX`) · 직전과 같은 내용(`histSig` = 메타·형식·조립 텍스트)은 건너뜀 · 30일 지난 것은 부팅 30초 뒤 `histPruneAll()` 이 정리 · 새 설교가 첫 저장으로 ID를 받으면 `hist:_new` 를 그 ID로 흡수(`histAdoptNew`).
- ★ 이력 쓰기는 **기다리지 않고 실패를 삼킨다** — 저장을 막거나 늦추면 안 된다(IDB 쓰기 실패 주입 시험: 저장은 정상 완료).
- 되돌리기(`histRestore`)는 작성창만 바꾸고 `savedSig` 는 두므로 "미저장" 상태가 된다 — [저장]해야 시트 반영.
- 비교는 AI 없는 줄 단위 LCS(`histDiff`/`histDiffHtml`, 앞뒤 공통 줄을 걷고 가운데만, 400만 칸 초과 시 통째로 '바뀜'). ⚠ 계획서의 "`cmpHtml` 재사용"은 틀렸다 — `cmpHtml` 은 「준비 원고 vs 실제 설교」 AI 분석 결과 렌더러다.

## 전환표시 HWPX에 생성 이미지 넣기 — v588

「전환 표시 HWPX」가 전환 표시 PDF 처럼 **전환 바마다 그 번호의 생성 이미지**를 얹는다(버튼 추가 없음). 생성 이미지가 하나도 없으면 예전과 똑같은 문서가 나온다.
- `transHwpxBlob(meta, body)` — 전환표시본 HWPX 를 만드는 세 곳(캐스케이드 `doTrans`·단독 `generateTransDoc`·원고 수정 반영 `applyRetrans`)이 공용. 이미지 출처는 PDF 와 같은 **`tpdfImgMap`**(백스테이지 zip 루트 + 낱장 재생성 보완) — 두 산출물이 늘 같은 그림을 쓴다. 표지(#1)는 PDF 처럼 제목부 아래 첫 빈 줄 자리에 가운데 정렬 24% 폭.
- **HWPX 그림 삽입(이 앱 최초)** — `hwpxPicXml()` 은 한글이 직접 저장한 문서(수련회 기도회 hwpx)의 `hp:pic` 구조를 그대로 따른다: 원본 크기 = 픽셀×75(HWPUNIT, 96dpi), `hc:img binaryItemIDRef` → `BinData/imageN.jpg`(STORE) + `Contents/content.hpf` 에 `opf:item … isEmbeded="1"` 등록(header 항목 바로 뒤). `hwpxBuild(content)` 가 블록의 `img.u`(dataURL)를 모아 처리하므로 **다른 문서에도 블록에 `img` 만 붙이면 그림이 들어간다**(`role:'pic'` = 가운데 한 줄 그림).
- 배치: 그림은 떠 있는 개체(글 앞으로·단 기준 오른쪽 정렬, 폭 = 바의 21% `HWPX_BAR_IMG_RATIO`). 그림이 바보다 높으면 **바 표의 바깥 여백 위·아래를 (그림높이−바높이)/2 로 늘려** 자리를 만든다 → 그림이 바 세로 가운데에 오고 앞뒤 글을 덮지 않는다(음수 오프셋 없이).
- ⚠ 캐스케이드 순서상 전환표시본(`doTrans`)이 배경 이미지 생성(`doBg`)보다 **먼저** 돈다. 그래서 같은 실행에서 두 단계가 모두 켜져 있으면 이미지 생성 뒤 **같은 본문으로 HWPX 를 다시 저장**한다(AI 호출 없음).
- 검증: 한컴오피스 한글(맥)에서 실제로 열어 확인 — 표지·바 3개 이미지가 바 오른쪽 끝 세로 가운데에 표시, 이미지 없는 #5 는 바만, 이미지 없는 문서는 예전과 동일. 그림이 든 파일을 `zipDocParas` 로 되읽어도 본문 줄이 그림 없는 파일과 **완전히 같다**(`ensureTransBody` 재사용 경로 영향 없음).

## 본문 속 번호 목록이 대지로 오인되던 문제 (v587)

2026-09-25 제보: 대지 1 본문에 「…하나님의 명령은 무엇이었습니까? / 1. 라합과… / 2. 성과…」 같은 목록이 있으면 구조 창에 **「2. 성과…」가 대지 2로** 떴고, 자유원고→기본원고 전환 시 대지 1이 「1. 라합과」에서 **잘린 것처럼** 보였다. 원인은 v572 규칙 — 첫 대지 이후 번호 줄은 「다음 대지 번호」일 때만 대지로 보는데, 목록의 「1.」은 걸러졌지만 **「2.」가 마침 다음 대지 번호(2)와 같아 승격**됐다. 그 뒤로 대지가 한 칸씩 밀리고 진짜 대지 2는 본문으로 삼켜졌다.
- **`parseFreeToStructure` 의 목록 추적(`subNext`)** — 대지로 안 본 번호 줄이 나오면 목록이 열린 것으로 보고, 다음 번호가 **빈 줄 없이** 이어지면 목록 항목으로 둔다. 목록은 「빈 줄 뒤 번호 없는 글」이나 구조 코드 줄(`(주해)`·`ex)` 등)에서 닫힌다. **빈 줄 바로 뒤의 「다음 대지 번호」는 대지로 본다**(대지 머리는 늘 빈 줄 뒤에 온다 — `assembleText` 도 그렇게 조립).
- 근거(원고 1,121편 실측): 본문 속 목록 78편 중 「2.」가 뒤따르는 65편 **모두** 항목 사이에 빈 줄이 없었다. 수정 전후 대지 목록이 바뀐 원고 25편 — 대부분 기도제목·요약·이유 목록이 가짜 대지 3·4로 붙던 것이 바로잡혔고, 왕복 가드 결과(구조로 열림/자유원고)는 0편 변동. 빈 줄 없이 쓴 옛 원고 두어 편(#1024·#575)은 전후 모두 모호하다.
- ⚠ 남은 한계: 목록 항목 사이에 **빈 줄을 두고**, 그 번호가 마침 다음 대지 번호와 같으면 여전히 대지로 본다(실측 0편). 대지 머리를 `(대지)` 코드로 쓰면 번호와 무관하게 확실하다.
- 비교 도구: `tools/points_dump.py`(roundtrip_test 와 같은 방식으로 파서를 떼어 원고별 대지 목록을 뽑음) — 파서를 고칠 때 전후 JSON 을 비교하면 된다.

## AI 모델 목록 — 자동 갱신 + Opus 5.5 (v586)

설교 대화의 모델 선택지가 코드에 박혀 있어 새 모델이 나와도 고를 수 없었다(2026-09-23 Opus 5.5 제보).
- **목록 자동 갱신** — 대화 패널 초기화 때 `aiModelsSync()` 가 **Models API(`GET /v1/models?limit=100`)** 로 계정이 실제로 쓸 수 있는 모델을 받아, 박힌 목록에 없는 것을 「… · 새 모델」로 덧붙인다. 하루 한 번만 조회(`localStorage['tsa.modelsCache']`), 실제 키가 없거나(브라우저 에이전트 모드) 실패하면 **조용히** 박힌 목록만 쓴다. 날짜 스냅샷 ID(`…-20251001`)와 옛 세대(`claude-3…`)는 감춘다. → 다음 모델이 나오면 앱 수정 없이 목록에 뜬다.
- **박힌 목록(정본·순서 유지)**: Opus 5(기본) · **Opus 5.5** · Sonnet 5 · Haiku 4.5. Haiku 는 날짜 없는 정식 ID `claude-haiku-4-5` 로 바꿨고(예화 인식 호출도 함께), 저장된 옛 ID 는 `AI_MODEL_MIGRATE` 가 승격한다.
- **`modelExtra(model)`** — 모델별 추가 파라미터. **Opus 5.5 는 기본 effort 가 `medium`**(Opus 5 는 `high`)이라 `output_config:{effort:'high'}` 를 붙인다. effort 를 모르는 옛 모델(Haiku 4.5 등)에는 붙이지 않는다(400). `callClaude`·`callChat` 이 공용.
- 생성 기능(요약·원고·카드 등)의 기본 모델은 여전히 `AI_MODEL = 'claude-opus-5'` 고정이다 — 대화 선택은 대화에만 적용된다.
- ⚠ 모델 관련 사실(ID·가격·기본 effort·제약)은 기억하지 말고 **`claude-api` 스킬**에서 확인할 것. Opus 5.5 는 thinking 끄기 불가·강제 tool_choice 400·prefill 400 인데, 앱은 셋 다 쓰지 않아 호출 형태는 그대로 호환된다.

## 기기 간 이어쓰기 — 대기열 덮어쓰기·기준 시점 오인 (v585)

2026-09-23 제보: 사무실 → 집 → 사무실로 이어 쓰면 ①사무실 작성창이 집 최종본을 안 받고 옛 로컬본을 계속 보여 주고 ②그 상태에서 [동기화]를 누르면 **옛 내용이 시트로 올라가 집에서 고친 내용을 덮었다.** 가짜 시트(`tools/fake_sheet.py`, 이 세션에서 다시 만듦)로 두 기기를 재현해 원인 두 개를 확인했다.

- **① 대기열이 시트를 무조건 덮었다(유실의 직접 원인).** 저장이 일시 통신 오류(30초 타임아웃 등)로 대기열에 들어가면 앱은 「저장됨」으로 표시하고, 그 항목은 **기준 시점 없이** 다음 `flushQueue`(모든 `pullData` 앞)에서 시트를 통째로 update 했다. 그 사이 다른 기기가 저장했어도 경고 없이 사라졌다.
  → 이제 `qStampBase_` 가 큐에 넣을 때 `_base`(그 버퍼가 기반한 `수정일`)를 새기고, `qGuardBody_` 가 보낼 때 시트를 확인한다: **그대로면 전송 / 내용이 이미 같으면 건너뜀**(타임아웃이지만 실제로는 저장된 경우) / **더 새 저장이 있으면 덮지 않고 「검토본」 행(`충돌원본`)으로 올린다**(둘 다 보존 + 토스트). 대상은 `원고텍스트` 가 든 `sermons` update 만 — 요약본 등 부분 update 는 서버가 칸 단위로 병합하므로 그대로 둔다.
- **② 기준 시점(`_bufMod`)을 남의 행에서 받아 왔다.** 저장 성공은 `_bufMod` 를 null 로 두고 `bufModAdopt` 가 **다음에 들어온 아무 행**의 `수정일` 을 기준으로 삼았다. 저장 뒤 동기화 전에 다른 기기가 저장하면 그 새 행이 내 기준이 돼 `reconcileCleanDraftWithSheet`·`_maybeFlagConflict` 가 영영 서지 않았다(= 작성창이 옛 내용 그대로, 경고도 없음).
  → 저장 성공이 `state._bufMod = nowLocalStr_()` 로 **직접** 기준을 찍는다(세 경로: 저장·새 설교로 저장·검토본). `bufModAdopt` 는 **깨끗한 버퍼 + 내용이 같은 에코**만 받는다(`composeSameAsRow`).
  → 기준이 아예 없는 옛 상태에서는 **깨끗한 버퍼면 시트를 정본으로 본다**(`newer = !qHasPendingManuscript_(id)`). 단 이 기기에 그 설교의 **미전송 대기분이 있으면 건드리지 않는다**(시트가 내 최신보다 옛것이므로).
- 검증(가짜 시트 2기기): 대기열 충돌 → 집 최종본 보존 + 검토본 생성 / 이미 반영된 대기분 → 건너뜀 / 충돌 없는 대기분 → 정상 전송 / 저장 후 남의 새 저장 → 작성창이 최신본을 받음 / 기준 없는 옛 상태 → 최신본 수신 / 미전송 대기분 있을 때 → 작성창 보존.
- ⚠ 남은 것: 대기열에 원고가 남아 있는 동안에는 그 설교를 다른 기기에서 이어 쓰지 않는 편이 안전하다(검토본으로 갈라진다). 사이드바 대기열 배지가 미전송 건수를 보여 준다.

## 빠진 원격 변경 복구 · 전체 로드 본문 보존 — v577

2026-09-22 사무실 맥에서 S1141 제목이 집 맥 저장(9/21 17:4x) 이전 것으로만 보였다. 원인은 v576 이전 코드 — 9/22 04:14 동기화 때 S1141 이 미저장 편집 중이라 원격 행을 버리고 기준점만 옮겼다(Aside 브라우저의 앱 IndexedDB 에서 확인: 로컬 행 수정일 13:39, 기준점 13:38→04:14, 새 제목 기록 0회). v576 보류분은 그 뒤의 변경만 지키므로 이미 빠진 행은 델타로 영영 돌아오지 않는다.
- **1회 전체 새로고침** — 부팅 때 `localStorage['tsa.resync577']` 이 없고 캐시가 있으면 `state.syncAt=''` 로 전체 목록을 받는다. `_pullData` 가 전체 응답(`!d.delta`)을 받으면 표시를 남긴다(설정이 없거나 실패하면 다음 실행에 다시). 편집 중인 설교는 전체 로드에서도 보류분으로 처리된다. ⚠ 같은 사고를 다시 전 기기에서 치유해야 하면 **키 이름을 바꿔** 한 번 더 돌린다.
- **전체 로드도 본문 캐시 보존** — 목록엔 원고 본문이 없어 전체 로드 한 번에 기기의 본문 캐시가 전부 사라졌다(오프라인 강단에서 못 엶). 이제 받아 둔 본문을 옛 `_txtAt` 째 옮겨 붙이고, 수정일이 바뀐 행은 `_txtStale` 이 열 때 새로 받는다(수정계획 3단계 7번의 앞 절반).
- **열기 순서** — `window.editSermon`·`resumeSermon` 은 확인창 통과 직후 `heldApply(true)` 를 먼저 한다. v576 은 `loadSermonThen`(=`ensureText`)이 옛 행 기준으로 "본문 최신"이라 판단한 **뒤에** `editSermonFill` 안에서 보류분을 적용해, 다시 연 설교가 **제목은 새것·본문은 옛것**이었다. 미리보기·내보내기처럼 버퍼를 버리지 않는 `loadSermonThen` 호출에는 넣지 않는다(편집 중인 설교의 기준 행을 바꾸면 충돌 판정이 틀어진다).
- 검증(가짜 시트): 변경을 놓친 상태를 IndexedDB 로 재현 → v577 첫 실행이 `since=''` 로 한 번 받고 제목 복구 · 열 때 새 본문 수신 · 다른 설교 본문 캐시 유지 · 두 번째 실행은 델타 · 보류분 있는 설교 재열기 시 제목·본문 모두 새것.

## 대지 = 청중이 붙들 변화의 지침 — v581

앱이 만드는 모든 설교의 대지는 **본문 요약이 아니라 청중의 변화 지침**(사고·관점의 전환 / 행동의 변화 / 새로운 이해)이어야 한다(2026-09-22 목사님 지시). 공용 규칙 **`POINT_CHANGE_RULE`**(`METAPHOR_AXIS_RULE` 아래)을 대지를 세우거나 고치는 프롬프트 끝에 붙인다: `P_SERMON`(주일 시리즈)·`P_OCC`(상황)·`P_DAWN`(새벽)·`P_TOPIC`(주제 시리즈)·`P_ZONE`(구역권찰, P_MS 규칙 줄을 통해서도)·`P_MS`(완성 원고 — **입력 대지가 있으면 목사님 문장을 그대로 쓰고, 없을 때만** 이 원칙으로 세운다)·`P_REFRAME`(대지 보강)·설교 대화 `sysFor` 기본문.
- **v583: 세 유형(관점 전환·행동 변화·새 이해)은 쓰는 방식이지 채울 칸이 아니다.** v581~582 에서는 「대지 셋은 서로 다른 변화를 겨냥」 문구와 구역권찰 스키마의 `변화` 필드 때문에 대지마다 유형이 하나씩 기계적으로 배정됐다(목사님 지적 — 본문 메시지에서 벗어날 위험). 지금은 본문 메시지가 먼저, 같은 유형 반복·유형 누락 허용, `변화` 는 대지를 다 세운 뒤 붙이는 이름표. **유형 배분을 다시 요구하는 문구를 넣지 말 것.**
- 규칙이 기존 「뼈만 남은 명제·함축적 표현」보다 **우선**한다고 명시했다. 예전 문구 「청중이 물음표를 품게」는 「청중이 붙들 변화의 방향이 분명하게」로 바꿨다.
- ⚠ **새로 대지를 만드는 프롬프트를 추가하면 `+ POINT_CHANGE_RULE` 을 반드시 붙일 것.** 카드원고(`P_CARD`)·요약본(`P_SUM`) 등 이미 정해진 대지를 옮기는 프롬프트에는 붙이지 않았다(목사님 대지를 바꾸면 안 되므로).

## 구조 창 서론·결론 상자 — v580

설교 작성 왼쪽 구조 창에 대지 상자만 있던 것을 **서론 상자(위)·결론 상자(아래)** 로 넓혔다(`#introBox`·`#concBox`, `renderIntroConcBoxes()` — `renderPoints()` 가 함께 부른다. `#pointList` 밖이라 대지용 드래그·타입변경 핸들러와 섞이지 않는다).
- **서론 = 여전히 문자열 `c.intro` 하나.** 원고텍스트 소비처가 66곳이라 모델을 바꾸지 않고, 화면에서만 `[서론 내용 | 도입 예화 | …]` 조각으로 나눠 보여 준다(`introSegsParse`/`introSegsJoin`/`introSegs(c)` 캐시/`introCommit`). 도입 예화 = `ex) 제목` + 본문, **예화 뒤에 서론 글을 이으면 `(서론)` 코드로 시작**한다(없으면 파서가 예화 본문으로 본다 — 대지의 `(주해)` 마커와 같은 이유).
  → ⚠ 그래서 `parseFreeToStructure` 가 서론의 `(서론)` 코드를 **더 이상 지우지 않는다**. 내보내기는 코드를 찍지 않는다(`parasFromText` 는 원래 지웠고, `parasFromCompose` 는 조각 단위로 찍으며 도입 예화 제목을 대지 예화처럼 파랑 굵게). 실측: 기존 원고 399편 중 `(서론)` 사용 0편, 서론에 `ex)` 가 있는 원고 134편 — 이제 그 예화가 서론 예화 칸으로 열린다.
- **서론 제목(`c.introTitle`)은 구조 창 이름표** — 원고(미리보기·HWPX·카드 등)에 찍지 않는다(2026-09-22 목사님 결정). 원고텍스트 밖이라 **`파일맵.introTitle`** 에 둔다: 저장 성공 뒤 `histAfterSave` → `introTitlePersist`(바뀐 때만, `apiPostOrQueue`), 불러올 때 `editSermonFill` → `sermonIntroTitle`. 작성 버퍼에 있으므로 초안·이력에도 따라간다. `composeSig` 가 제목을 따로 붙여 미저장 감지(제목이 비면 예전 서명과 같다).
- **결론 상자**는 정리 문구(`c.concIntro`) 입력 하나 + 결론 내용 미리보기. 오른쪽 결론 첫 칸과 서로 동기.

## 구역권찰 설교 구상 — v579

금요 구역장·권찰 예배 설교(3대지) 초안. 기초 자료 = 지난 주일 담임목사님(영락교회) 설교 + 그 설교로 **소그룹 사역 전담 사역자들이 만든** 구역장용 소그룹 교재(대학부 LTC 와 형식·내용이 다르다 — 교재 형식을 가정하는 파싱을 넣지 말 것, 통째 텍스트로 넘긴다). 메뉴 `data-page="zone"`, 페이지 `#page-zone`(`#zoneMeta`/`#zonePane`), 상태 `state.zone` → IndexedDB `zoneDraft`.
- **두 단계**: [📥 자료 읽기](`zoneRead`) → 오른쪽에 고칠 수 있는 텍스트로 남김 → [✍ 3대지 설교 구상](`aiZoneGen`, `P_ZONE`). 전사·이미지 인식은 느리고 비용이 드므로 재구상 때 다시 읽지 않게 나눴다.
- **v582: 완성 원고를 만들지 않는다.** 다른 구상 탭처럼 **개요 선택지 3세트**(`state.zone.opts`, `optEdit` 형식 + 대지별 `변화` 유형)만 만들고, [이 개요로 설교 작성](`zonePick(i)`, `occPick` 과 같은 방식: 주제근거→서론 바탕, 대지→구조 창)으로 작성 탭에 넘긴다. `P_ZONE` 은 구역권찰 전용 줄 + `P_OCC` 공통 줄(상황 전용 줄 제외)이다. 선택지는 카드로 시각화(`zoneVisHtml`: 제목·본문 알약·핵심메시지 강조·대지 카드 3장[변화 배지 관점 전환/행동 변화/새 이해·근거·적용]·주제근거), 수정은 카드 아래 [✏ 개요 다듬기]에서 하고 카드가 즉시 다시 그려진다. v579~581 의 완성 원고 초안(`Z.draft`)은 새로 구상할 때 버린다.
- **주일 설교 입력**: 유튜브 → `ytTranscribeInApp`(Gemini 키 필수, 워커 폴백 안 씀) + oEmbed 제목에서 제목·본문 추출(`zoneYtMeta`) / 홈페이지 → `zoneFindPageImgs` / 이미지·PDF 파일 직접 업로드.
  → ⚠ youngnak.net 은 **CORS 가 없어** 앱이 페이지·이미지를 읽을 수 없다. 그래서 이미지 주소를 **파일명 규칙** `/wp-content/uploads/<게시 연/월>/<설교일 YYYYMMDD>_001.jpg…` 으로 만들고 `<img>` 로드로 존재만 확인한 뒤, Claude 에 `source:{type:'url'}` 로 넘겨 **Anthropic 서버가 가져가게** 한다(`zoneOcr`, Sonnet 5). 교회가 파일명 규칙을 바꾸면 여기가 깨진다 — 그때는 파일 업로드로 우회.
- **교재**: HWPX·DOCX → `zipDocParas` / PDF → pdf.js 3.11.174(cdnjs) 글자층, 글자층이 없는 스캔 PDF 만 Claude `document` 블록. HWP(옛 한글)는 거절 안내.
- **v584: 본문 = 주일 설교 본문 그대로.** 프롬프트에 「필요하면 그 안의 핵심 단락」을 허용했더니 선택지마다 범위가 달라지고 주일 본문 밖 절까지 넣었다. 지금은 ①왼쪽 **[주일 설교 본문]** 칸(`#zone_ref` ↔ `Z.srcRef`, 유튜브 제목에서 자동 채움) ②비어 있으면 구상 전에 `uiPrompt` 로 확인(원고 앞 2,000자에서 `BOOKS` 성구를 찾아 기본값) ③프롬프트 ①번 「글자 그대로, 줄이거나 넓히지 않는다」 ④**생성 뒤 코드가 세 선택지의 `본문` 을 그 값으로 덮어쓴다**(프롬프트만으로는 못 막는다 — 이 덮어쓰기를 지우지 말 것).
- 선택지마다 `refAudit` 로 대지·주제근거의 성경 절 실재성 검사. `msArrToFree`(v579에서 `aiManuscript` 에서 뽑아낸 공용 변환)는 이제 `aiManuscript` 만 쓴다.

## 전체 새로고침 타임아웃 — v578

[전체 새로고침]이 「timeout: 시트 응답이 없어 중단했습니다」로 자주 실패했다. 모든 시트 요청이 30초(`SHEET_TIMEOUT_MS`)에 끊기는데, 전체 목록(`list&sheet=all`, `since` 없음)은 설교 전 행의 `파일맵`(전환표·카드원고·LTC JSON)·요약본에 묵상·산출물·**대화기록 전체**까지 읽어 직렬화한 뒤에야 응답이 시작되기 때문(Apps Script 콜드 스타트가 겹치면 초과).
- `_sheetFetch(url, opts, ms)`·`api(params, ms)` 에 제한 인자를 두고, `_pullData` 가 **`since` 없는 요청에만 `SHEET_FULL_TIMEOUT_MS`(120초)** 를 준다. 델타·저장은 그대로 30초.
- `fullResync()` 는 더 이상 `syncAt=''` 을 **미리 저장하지 않는다** — 실패하면 기준점이 빈 채 남아 이후 포커스 동기화·다음 실행이 매번 무거운 전체 요청을 보냈다. `{full:true}` 만으로 전체 조회가 되고, 성공 시 `_pullData` 가 기준점을 새로 잡는다.
- 남은 과제(백엔드, 수동 배포): `Code.gs` 에서 `sheet=all` 일 때 `chats` 제외(프런트는 `sheet=chats` 로 따로 받는다).

## 기기 간 덮어쓰기 방지 ① — v576 (수정계획 3단계 a: 1·2·6번)

- **보류분(`state._held`)** — `mergeDelta`·전체 로드는 미저장 편집 중인 설교(`_editingGuardId`)의 원격 행·삭제를 반영하지 않는데 `syncAt` 은 전진한다. 예전엔 그 변경을 그냥 버려서, 편집을 버리거나(새 설교·다른 설교 열기) 검토본으로 저장한 뒤에도 그 설교의 로컬 행이 옛 내용으로 남았다(→ 옛 본문으로 이어 쓰다 다른 기기 저장을 덮음). 이제 `heldPut` 으로 보관하고 **IndexedDB `heldRows`** 에도 쓴다(앱을 닫아도 유지, 부팅 때 복원). `heldApply(force)` 가 보호가 풀린 보류분을 적용 — `mergeDelta` 맨 앞(비강제), `clearCompose`·`editSermonFill` 맨 앞(강제: 버퍼를 버리는 경로). 로컬 `수정일` 이 더 최신(내가 덮어써 저장)이면 버린다. 전체 로드는 보류분을 비우고 편집 중 행만 다시 보류하며, **전체 목록에 없는 편집 중 행은 원격 삭제로 보류**한다(예전엔 로컬 행을 되살렸다).
- **기준 시점(`state._bufMod`)** — 깨끗한 작성창은 다른 기기 저장을 받지 못했다(`_draftBaseMod` 가 복원 초안에만 있어 `reconcileCleanDraftWithSheet` 가 늘 `newer=false`). 이제 `editSermonFill` 이 열 때 행의 `수정일` 을, 저장 성공(`clearComposeDraft`)이 null 을 넣고, 비어 있으면 동기화로 들어온 그 설교 행(서버 값)을 `bufModAdopt` 로 받는다(= 내 저장 에코). 복원 초안(`_draftBaseMod`·`_draftAt`)이 있으면 손대지 않는다.
  → `reconcileCleanDraftWithSheet` 의 기준 = `_draftBaseMod || _bufMod`. 더 새 행이 오면 본문을 받아 **`composeSameAsRow`(메타 칸 + 조립 원고)가 같으면 기준만 옮기고 조용히 넘어간다** — 내 저장 에코·시계 차이로 작성창을 다시 그려 커서가 튀지 않게. ⚠ 이 동등 비교를 지우지 말 것.
  → `_maybeFlagConflict` 도 `_draftBaseMod || _bufMod` 를 기준으로 삼아, 미저장 편집 중 **본문만 바뀐** 다른 기기 저장도 충돌로 표시한다. 저장 직후엔 `_bufMod` 가 비어 있고 에코를 먼저 `bufModAdopt` 하므로 자기 에코로는 켜지지 않는다.
- **기준점 보호** — `idbSet` 에 `tx.onabort`(쿼터 초과는 onerror 없이 abort 로 끝나 프라미스가 안 끝났다). 부팅 때 `syncAt` 은 **`state.data.fetchedAt`(데이터와 같은 레코드)과 IDB `syncAt` 중 이른 쪽**, IDB 값이 `''` 이면 전체 새로고침 요청으로 존중. `focusSync` 는 `_dataLoaded` 전엔 돌지 않는다.
- **검증 도구** — `Dropbox/total-sermon/tools/fake_sheet.py`(Code.gs 의 list/text/add/update/delete 를 분 단위 `수정일`·델타 규칙 그대로 흉내, `/_tick` 으로 시계 앞당김). 앱을 로컬 서버로 띄워 `127.0.0.1` 과 `localhost` 두 탭(저장소가 분리됨)을 두 기기로 쓴다. 로컬 서버에서는 이 브라우저가 서비스워커 등록을 거부해 콘솔 오류가 나는데 앱과 무관하다.

## 원고 자체 점검 — v575 (수정계획 11단계 A, 앞당김)

작성 화면 「📕 성경 참조 전체 검증」 아래 **[✍ 원고 자체 점검]**(`#selfAuditBtn` → `#ptSelfAudit`). 설교 작성 원리의 "내보내기 전 자체 점검"을 **AI 없이** 센다(v548 검증 게이트와 같은 발상). 진입점 `selfAuditCompose()`.
- ① **반복어** — `saWordsCfg()`: 「단어:전체 허용/대지당 허용」 목록, 기본 `SA_WORDS_DEF`(자리 3/1 · 머물·붙들·그 순간·무너 3 — 지침 5장 16번). 결과 칸의 입력칸에서 고치면 **기기별** `localStorage['tsa.saWords']`(기본값과 같거나 비우면 삭제). 성경 인용 구간은 `saBlank` 로 공백 처리해 셈에서 뺀다. 「자주 쓴 말(참고)」은 조사 떼기 + 불용어(`SA_STOP`) 후 6회 이상 상위 8개 — 경고 수에 넣지 않는다.
- ② **성경 인용 전문** — 인용 찾기는 `refAnyRe()`(빨간 볼드와 같은 판별), 절 전문은 `fullVerseText()`(bible/bNN.json). 책 없는 「3:20」은 작성 화면 본문칸의 책, 「N절」은 본문이 한 장일 때만. 판정 `saCheckQuote`: 일치 / 부분(a·b 표기면 '참고', 아니면 경고) / **중략은 「…」 앞뒤 조각이 절 안에서 실제로 건너뛸 때만**(「나사렛에서 … 나사렛에서」 같은 되뇌기는 중략 아님) / 전문 뒤에 덧붙인 반복은 '참고' / 그 밖은 일치도(`similarity`)로 「글자 다름(≥80%)」·「다른 번역·의역」.
- ③ **대지 마무리** — 대지 구조는 `saStructView()`(구조 모드면 버퍼, 자유원고면 `composeFreePoints` 방식의 임시 파싱본 — **작성 버퍼는 건드리지 않는다**). 대지마다 주해근거·예화·대지적용·대지실천·대지정리 유무, 마지막 블록이 정리인지, 정리에 권면(`SA_EXHORT`)이 있는지, 갈무리끼리 비슷한지(`saGalNorm` 으로 「~이 아니라 ~입니다」 같은 공통 틀을 뺀 뒤 일치도 ≥60%).
- ④ **분량** 공백 제외·분당 300자(리허설 타이머와 같은 기준), 대지 편차 1.8배 초과, 결론 본문 6문장 초과. ⑤ **긴 문장** `SA_LONG`=60자 초과(인용·카드원고 전환 바 제외).
- 항목을 누르면 `saJumpTo()` 가 `#editorPane` 의 입력칸을 화면 순서대로 뒤져 그 글을 선택하고 스크롤한다(다시 누르면 다음 위치).
- 결과는 설교 전환 때 `clearAiPanels()` 로 함께 지운다.
- 실측(2026-06-07 요 1:43-51 카드원고 6,713자): 0.55초 · 「자리」 20회 · 인용 8곳 중 경고 1곳(1:51 절 중간 삽입), 참고 4곳(a/b 부분 인용·반복 강조).

## 유튜브 전사 — 「예배 전체」가 설교를 놓치던 문제 (v569)

예배 실황 전체를 넣었는데 **사도신경·찬양 622자만 전사되고 끝났다**(2026-09-20, S1332). 원인은 「예배 전체」 모드만 **스캔 결과를 검증 없이 믿고, 끝까지 훑는 안전장치에서 빠져 있던 것**이다.
- 스캔(설교 시작·끝 시각을 묻는 1회 호출)의 검사라고는 `끝 > 시작`·`끝 ≤ 길이` 뿐이라 「0초~5분」 같은 답도 통과했고, 그 구간만 받아쓴 뒤 그대로 끝났다.
- 네 가지를 고쳤다. ①**훑기를 두 모드 모두에 적용**(`ytTranscribeInApp` ③ 블록의 `serviceType !== 'full'` 조건 제거) — 시작 지점이 틀려도 결국 설교를 잡는다. ②**스캔 상식 검사** — 설교 구간이 12분 미만이면 스캔 실패로 돌려 영상을 처음부터 훑는다. ③**스캔에서 사고를 켰다**(`thinkingBudget: 0` 제거 + `maxOutputTokens` 2048→8192 — 사고 토큰이 같은 한도에서 빠지므로 함께 올리지 않으면 JSON 이 잘린다). ④**분량 상식 검사** — 전사문이 2,000자 미만이면 시트에 쓰기 전에 `uiChoose` 로 묻는다(예전엔 조용히 저장하고 요약까지 갔다).
- ⚠ 대가: 「예배 전체」도 설교 구간 밖까지 훑으므로 **Gemini 호출이 늘어 시간·비용이 대략 두 배**다. 멈춤은 빈 물결(30분) 기준이며 `CAP`(30창=5시간) 안전판이 여전히 상한이다.
- 워커(`worker.js`)의 `/youtube` 라우트에는 같은 허점이 그대로 남아 있다 — 키가 있으면 앱 직접 경로만 쓰므로 방치했다.

## 유튜브 전사 — 키 없으면 먼저 막는다 (v568)

`aiYouTubeSummarize` 는 설정에 **Gemini 키가 있으면 앱에서 직접**, 없으면 프록시 워커 `/youtube` 로 폴백한다. 그런데 **워커 폴백은 지역 차단으로 사실상 항상 실패**한다(구글이 클라우드플레어 해외 엣지를 막는다 — 결제 연결 키도 동일, 위 v517 항목 참조).
- 2026-09-21 사례: 맥에 Gemini 키가 없어 워커로 넘어갔고 「전사 실패: Gemini 400: User location is not supported for the API use.」만 떴다. 이 문구는 **워커에서만** 나온다(`worker.js` 의 `Gemini ${status}: ${msg}`) — 앱 직접 호출 경로였다면 `gemVideo` 가 우리말 안내로 바꿔 준다. 즉 **이 문구가 보이면 그 기기에 키가 없는 것**이다.
- 지금은 ① 키가 없으면 시작 전에 `uiChoose` 로 알리고 [⚙ 설정에서 키 넣기]로 설정 화면을 열며(프록시가 설정돼 있으면 [그래도 프록시로 시도]도 남겨 둔다) ② 워커가 그 오류를 돌려주면 우리말 안내 + 설정 화면으로 보낸다.
- **Gemini 키는 기기마다 따로**(로컬 저장, `XFER_KEYS` 로 수동 이전)다 — 아이패드에 넣었어도 맥에는 없을 수 있다.

## 작업 규칙
- 앱 코드 변경 후: 인라인 `<script>` 블록 문법 재검사(현재 3블록) + 위 두 버전 +1. 한 번에 확인하려면 `bash Dropbox/total-sermon/tools/smoke.sh`(문법+버전일치+샤드 정합). 백업 정리는 `tools/cleanup_baks.sh`(종류별 최근 5개 유지).
- 큰 변경 전 `index.html.bak-*` 백업 생성(`.gitignore`로 추적 제외됨).
- 백엔드(`Code.gs`) 변경은 수동 반영: 구글시트 → 확장 → Apps Script에 전체 붙여넣기 → `setup` 실행 → 배포 관리에서 기존 웹앱 **"새 버전"**으로(URL 유지).
- 멀티기기 동기화 설계·문제·해결책은 `Dropbox/total-sermon/docs/멀티기기_사용문제_진단_20260623.md` 참고.

## ⚠ 자동 커밋·푸시 워처 — 이 저장소에 git 명령을 직접 돌리지 말 것 (중요)

이 폴더에는 **맥에서 자동 커밋·푸시 워처가 상시** 돌며 변경을 origin/main에 푸시 → 배포한다. 이 워처가 인덱스(`.git/index`)를 수시로 건드린다.

**작업 지침(반드시 지킬 것):**
- 코드 변경은 **파일 편집(Read/Write/Edit)만** 한다. 커밋·푸시는 **워처에 맡긴다.** 굳이 즉시 올려야 하면 *사용자에게* 수동 `git add/commit/push`를 안내한다(에이전트가 샌드박스에서 직접 돌리지 않는다).
- **에이전트는 이 저장소에서 git 명령(`status`/`add`/`commit`/`reset`/`rev-list`/`fetch` 등 인덱스·refs를 건드리는 것)을 샌드박스 bash로 실행하지 않는다.** 이유: 워처와 동시에 실행되면 인덱스 잠금 충돌이 나고, 샌드박스↔맥 마운트가 파일 삭제(unlink)를 막아 `git`이 자기 `.git/index.lock`을 못 지운다 → **고아 잠금이 남아 워처의 자동 커밋이 영구히 멈춘다.**
- 배포 반영 여부·버전 확인은 git 대신 **파일 내용으로** 확인한다(예: `grep "var APP_VER" index.html`, 코드 식별자 grep, 파일 diff). 원격 상태가 정말 필요하면 사용자에게 물어본다.

**증상과 복구:** 변경이 origin에 안 올라가고 자동 푸시가 멈춘 듯하면 십중팔구 **stale `.git/index.lock`**이다. 맥 터미널에서 `rm -f .git/index.lock` 한 번이면 워처가 재개된다. (이 잠금 파일 자체는 정상이며 평소엔 git이 만들고 즉시 지운다 — 워처 단독 동작 시엔 문제 없음. 고아 잠금은 에이전트가 샌드박스 git을 끼어 돌렸을 때만 발생했다.)

## HWPX(한글) 산출물 — v493+

앱은 원고·카드원고·LTC 교재·설교계획·이미지자료·전환표시본을 **hwpx로 직접 생성**한다(기본값).
- 껍데기 템플릿: `total-sermon-web/hwpx/templates/{manuscript,dawn,card,ltc,plan}.hwpx`
  = 목사님 실제 원고에서 본문만 비운 파일. 용지·여백·글꼴·색·표 서식이 그 안 header.xml(charPr/paraPr/borderFill)에 확정돼 있다.
- 앱 코드: `index.html` 의 `HWPX_TYPES`(역할→ID 지도) + `hwpxBuild()`. **새 서식을 만들지 않고** 역할 ID로 문단만 찍는다.
- 같은 템플릿·같은 지도를 Cowork 스킬 **hwpx-sermon-docs** 도 쓴다(`hwpx/build_hwpx.py`, `hwpx/hwpx_types.json`).
  → 템플릿이나 역할 지도를 고치면 **앱·스킬 양쪽을 함께** 고칠 것. 템플릿은 두 벌
  (`total-sermon-web/hwpx/templates/`, `Dropbox/total-sermon/hwpx/templates/`)이므로 항상 같이 갱신한다.
- **템플릿의 `Preview/PrvImage.png` (v546, 2026-08-31)**: hwpx 안의 이 그림이 곧 macOS Finder 의 문서
  썸네일이다. 예전 껍데기에는 1×1 짜리 **반투명 초록 픽셀**(RGBA 0,255,0,127)이 들어 있어, 그 1픽셀이
  아이콘 크기로 확대되며 앱이 만든 hwpx 가 전부 **초록색 박스**로 보였다(파일·열기 자체는 정상).
  5종 템플릿 모두 210×297 흰 종이 PNG 로 교체했다. **앞으로 템플릿을 새로 뜰 때 이 1×1 스텁이 다시
  들어가지 않게 확인할 것.** 이미 만들어진 파일은 `total-sermon/tools/fix_hwpx_preview.py <폴더>` 로 일괄 교정
  (1×1 짜리만 골라 바꾸므로 한글이 저장한 진짜 미리보기는 건드리지 않는다). Finder 아이콘이 그대로면
  `qlmanage -r cache && killall Finder`.
- **전환 표시 PDF (v529, v530에서 썸네일 축소)**: 전환표시본과 같은 본문·전환표에 「배경 이미지 생성」으로 만든 그림을 각 전환 바 오른쪽 끝에 작은 참고 썸네일(본문 폭 21%)로 겹쳐 올려 PDF로 조판한다(음수 여백 flex — 그림 때문에 쪽수가 늘지 않게)(`tpdfImgMap`/`tpdfBlocks`/`buildTransPdfBlob`/`saveTransPdf`, 캐스케이드 키 `transpdf`). hwpx가 아니라 html2canvas+jsPDF로 만들지만 용지·여백·색은 hwpx 전환표시본과 같은 규격(216×290mm·여백 10mm·12pt·바 #EE0000). 이미지는 저장된 백스테이지 zip(NN.png)에서 읽으며, 없으면 만들지 않고 안내한다.
- **전환 표시 PDF의 보이지 않는 텍스트층 (v534)**: 페이지 래스터 위에 `renderingMode:'invisible'` 로 같은 글자를 같은 자리에 얹어, 아이패드 PDF 뷰어에서 하이라이트·밑줄·검색·복사가 되게 한다(스캔본 OCR과 같은 원리, 보이는 조판은 불변). 줄 위치는 `tpdfLineBoxes()` 가 DOM Range 로 글자마다 측정해 줄 단위로 묶고, `tpdfDrawTextLayer()` 가 px→mm 로 환산해 찍는다(자간은 charSpace 로 실제 폭에 맞춤, 글자폭 40% 초과 보정은 무시). 글꼴은 `fonts/pdf-text-layer-ko.ttf` — Noto Serif KR 한글 전체를 서브셋한 뒤 **외곽선을 지우고 자폭(hmtx)·cmap 만 남긴 220KB** 파일(보이지 않으니 모양 불필요 → PDF 용량 부담 최소). 없으면 텍스트층만 건너뛰고 예전처럼 만든다.
- **원고 수정 반영 — 전환표 재정렬 (v532)**: 산출물을 다 만든 뒤 원고 문장을 고쳤을 때, 전환표를 새로 만들지 않고 **컷 번호·개수·순서는 그대로 둔 채 앵커만 새 원고 문단에 다시 붙인다**(이미 만든 배경 이미지를 그대로 씀). 순서를 지키면서 전체 유사도 합이 최대가 되는 배치를 동적계획법으로 찾고(`retransMatch`), 전환 바 삽입은 AI 없이 로컬로 한다(`retransBody`). 진입점 넷 — ⓪**`ensureTransTable`(v533, 가장 중요)**: 표가 이미 있고 원고 서명만 달라졌으면 **AI로 새 표를 만들지 않고 재정렬**한다(예전엔 여기서 새 표가 나와 대지 선언 컷이 사라지고 번호가 한 칸씩 밀려 이미지와 어긋났다). 재정렬 실패분이 34%를 넘을 때만 새 표를 만들고, `force`([전환표 다시 잡기]·[다시 생성])는 그대로 새 표. 재정렬했으면 `window._transRealigned`로 컷 사이드카도 묻지 않고 같이 맞춘다 ①[♻ 원고 수정 반영] 버튼(`btnRetrans`→`applyRetrans`: 전환표·컷 사이드카 갱신 → 전환표시본 → 전환 PDF) ②컷 사이드카가 낡았을 때 뜨는 3지선다(`askCutsStale`, 기본값 "위치만 다시 맞추기") ③`ensureTransBody`가 옛 전환표시본을 재사용하기 전 신선도 검사(`transBodyFresh`·파일맵 `transBodySig`) — 문장이 하나라도 다르면 재사용하지 않고 재정렬한다. 앵커 단락이 삭제됐거나 유사도가 낮은 컷은 `weak`로 보고(같은 자리 겹침 허용 — 뒤 컷이 줄줄이 밀리지 않게).
- DOCX 생성 코드는 그대로 두었다. 설정 → 도구 → **산출물 파일 형식**에서 DOCX로 되돌릴 수 있다.
- 되읽기(카드원고 추출·LTC 되읽기·드래그 가져오기·폴더 스캔)는 `zipDocParas()` 가 docx·hwpx 를 모두 처리한다(옛 docx 파일도 계속 인식).
