...
- ⚠ 프록시 워커(`Dropbox/total-sermon/image-proxy/worker.js`)를 **수동 배포해야** 초광각이 실제로 나온다(`wrangler deploy`). 배포 전에는 워커가 초광각 요청을 16:9 로 떨어뜨리므로, 앱이 그 16:9 를 위아래로 잘라 초광각을 만든다(왜곡은 없지만 화각이 좁다).
